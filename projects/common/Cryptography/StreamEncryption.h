...
public:
    ...
    void Setup(BigNumber* key)
    {
        ...
        {
            _hmach = new HMACH(STREAMENCRYPTION_KEY_SIZE, const_cast<u8*>(_serverKey));
            _serverEncryptor.Setup(_hmach->CalculateHash(key));

            _hmach = new HMACH(STREAMENCRYPTION_KEY_SIZE, const_cast<u8*>(_clientKey));
            _clientDecryptor.Setup(_hmach->CalculateHash(key));
        }
