...
struct ServerPacketHeader
{
    ServerPacketHeader(u32 size, u16 command) : size(size)
    {
        ...
        if (BigPacket())
        {
            data[index++] = 0x80 | (0xFF & (size >> 16));
        }

        data[index++] = 0xFF & (size >> 8);
        data[index++] = 0xFF & size;
        data[index++] = 0xFF & command;
        data[index++] = 0xFF & (command >> 8);
    }
...
enum Opcode
{
    ...
    SMSG_CONNECT_TO = 0x50D, // uint32 ip, uint16 port, uint32 unk (Potentially Attempt count), uint8[20] hash (ip + port, seed=sessionkey)
